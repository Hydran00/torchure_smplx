...
inline auto transform_mat(Tensor &&R, Tensor &&t) -> Tensor {
    return torch::cat({torch::pad(R, {0, 0, 0, 1}),
                       torch::pad(t, {0, 0, 0, 1}, "constant", 1)},
                      2);
}
...
auto find_dynamic_lmk_idx_and_bcoords(const Tensor &vertices,
                                      const Tensor &pose,
                                      const Tensor &dynamic_lmk_faces_idx,
                                      const Tensor &dynamic_lmk_b_coords,
